...
- `wpa_ctrl/ssid.py` — the spelling of octet strings and secrets on the wire: `printf_decode`, the `Ssid` type (octets first; `.text` is strict-UTF-8-or-None), `quote` and the psk helpers.
- `wpa_ctrl/security.py` — what an AP advertises (`Security`, `parse_security`) and the configuring vocabulary (`KeyMgmt`, `Pmf`) — two spellings of the same ideas, deliberately separate.
- `wpa_ctrl/replies.py` — the reply types (`Network`, `ScanResult`, `Bss`, `Status`, `SignalPoll`, `Station`, `P2pPeer`, `PmksaEntry`, `BssMask`) and the two parsers (`parse_variables`, `parse_table`). Dict-subclass types keep every field the daemon sent; typed properties answer None for a field the mask or the daemon left out.
- `wpa_ctrl/dpp.py` — DPP parameter building: `DppConf`, `dpp_hex`, `dpp_channel`, `dpp_configurator_params`, `format_params`.
- `wpa_ctrl/client.py` — `WpaCtrl` itself: a typed method per documented command (including all 29 DPP commands) and the four `iter_*` walks. Anything undocumented goes through `request()` verbatim. Commands answering `OK`/`FAIL` raise `WpaCtrlCommandFailed`; `try_command()` returns a bool where refusal is expected. The package (`from wpa_ctrl import ...`) is the import path — `wpa_ctrl.client` holds only `WpaCtrl` and the `REPLY_*` constants.
